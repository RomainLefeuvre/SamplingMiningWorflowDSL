...
class HistWorkflowAnalysis(WorkflowAnalysis):
    ...
    def __init__(self, metadata: Metadata[T],top_x: int = -1,category:bool=True,sort: bool = False):
        super().__init__()
    ...
    def analyze(self, workflow, workflow_name: str = "main workflow", op_number: int = 1):
        ...
        analysis = HistAnalysis(self.file_path, self.metadata,self.top_x,self.category,self.sort)
        ...
        while op is not None:
            if isinstance(op, GroupingOperator):
                analysis.analyze(op.get_merged_output(),f"{self.metadata.name}_{workflow_name}_op{op_number}_output.png",f"Output of operator class {op.__class__.__name__}")
                ...
                analysis.analyze(op.get_output(), f"{self.metadata.name}_{workflow_name}_op{op_number}_output.png",  f"Output of operator class {op.__class__.__name__}")

...
def main():
    ...
    workflow = (WorkflowBuilder().input(JsonLoader(input_path,url,  creation_date,commits_metadata,pass_reaper,number_of_security_update_dependabot,use_multiple_dependency_management_bots,build_system,contains_package_json,stars))
                                ...
                                # Filter project with more than X stars
                                .filter_operator("stars >= 50")  # adjust threshold
                                # Filter project with a commit before June 1, 2019
                                .filter_operator("commits_metadata.get_first_commit_date() < date_to_epoch_day(2019, 6, 1)")
                                # Filter project with at least an update after June 1, 2019
                                .filter_operator("commits_metadata.get_total_commit() > commits_metadata.get_total_commit_at_date(1, 6, 2019)")
                                ...
                                # * Filter project having at least a dependabot security update targeting npm or yarn
                                .filter_operator(
                                    "number_of_security_update_dependabot > 0"
                                )
